...
class Features:
    ...
    def filter(
        ...
        if origin is not None:
            filtered = [
                f
                for f in filtered
                if ([f.origin] if isinstance(f.origin, Text) else f.origin)
                == origin
            ]
        if type is not None:
    ...
    def combine(
        ...
        features = (
            scipy.sparse.hstack([f.features for f in features_list])
            if arbitrary_feature.is_sparse()
            else np.concatenate([f.features for f in features_list], axis=-1)
        )
        return Features(
            features=features,
