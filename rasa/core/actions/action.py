    ...
    async def run(
        ...
        if message is None:
            if not self.silent_fail:
                logger.error(f"Couldn't create message for response '{self.utter_action}'.")
            return []
...
class RemoteAction(Action):
    ...
    async def _utter_responses(
        ...
        for response in responses:
            if generated_response := response.pop("response", None):
...
class ActionExtractSlots(Action):
    ...
    def _fails_unique_entity_mapping_check(
        ...
        return not form.entity_mapping_is_unique(mapping, domain)
    ...
    async def run(
        ...
        return await self._execute_validation_action(
...
def extract_slot_value_from_predefined_mapping(
    ...
    trigger_mapping_condition_met = tracker.active_loop_name is not None and (
        not active_loops_in_mapping_conditions
        or tracker.active_loop_name in active_loops_in_mapping_conditions
    )
